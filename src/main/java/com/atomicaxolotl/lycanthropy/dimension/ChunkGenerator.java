...
public class ChunkGenerator extends net.minecraft.world.gen.ChunkGenerator {
    ...
    public static final Codec<ChunkGenerator> CODEC = RecordCodecBuilder.create(instance ->
            instance.group(
                    RegistryLookupCodec.getLookUpCodec(Registry.BIOME_KEY).forGetter(ChunkGenerator::getBiomeRegistry),
                    SETTINGS_CODEC.fieldOf("settings").forGetter(ChunkGenerator::getSettings)
            ).apply(instance, ChunkGenerator::new));

    private final Settings settings;

    public ChunkGenerator(Registry<Biome> registry, Settings settings) {
        super(new BiomeProvider(registry), new DimensionStructuresSettings(false));
        this.settings = settings;
        Lycanthropy.LOGGER.info("Chunk generator settings: " + settings.getBaseHeight() + ", " + settings.getHorizontalVariance() + ", " + settings.getVerticalVariance());
    }
    ...
    public Registry<Biome> getBiomeRegistry() {
        return ((BiomeProvider)biomeProvider).getBiomeRegistry();
    }
    ...
    @Override
    public net.minecraft.world.gen.ChunkGenerator func_230349_a_(long seed) {
        return new ChunkGenerator(getBiomeRegistry(), settings);
    }
